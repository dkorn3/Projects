...
/* an entry in a tage predictor e.g. T[i] */
struct tage_entry_t {
	int8_t cntr;  /* 3-bit sat counter */
	uint8_t tag; /* 8 bit tag */
	uint8_t u;   /* 2 bit useful cntr */
...
struct bpred_dir_t {
  ...
  union {
    ...
    struct {
      int l1size;		/* bias table size (one entry per branch PC) */
      int l2size;		/* PHT size (number of agree counters) */
      int shift_width;		/* history register width */
      int xor;			/* history xor address flag */
      int *shiftregs;		/* level-1 history shift registers */
      unsigned char *l2table;	/* PHT: 2-bit agree/disagree saturating counters */
      unsigned char *bias_table;/* per-branch 2 bit counter */
    } agree; 
    struct {
      int table_size;
      int history_length;
     ...
     struct {
      struct bpred_dir_t *base; /* points to base, i.e. reuses bimod */
      char ret_ctr;
      struct tage_entry_t *T[4]; /* tables T1-T4 */
      int size;                  /* num of entries per table, 1024 */
      uint64_t ghist;             /* 40 bit global history */
      int hist_lengths[4];        /* 5, 10, 20, 40 */
      int clk;                    /* used for periodic useful (u) reset every 256 k branch */
    } tage;
  } config;
};
...
struct bpred_update_t {
  ...
  struct {		/* predicted directions */
   ...
   struct {              /* TAGE predictor state */
    int provider;       /* T[i] that gave prediction, -1 = base */
    int altprov;        /* T[i] that gave altpred,   -1 = base */
    int pred;           /* final prediction (1=taken, 0=NT) */
    int altpred;        /* alternate prediction */
    int provider_pred;     /* the prediction made by the prov, needed in update*/
    unsigned int idx[4];   /* save the computed idx and tags */
    unsigned char tag[4];
    uint64_t ghist;        /* save ghist */
  } tage;
};
