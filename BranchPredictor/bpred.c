	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredAgree:
    pred->dirpred.twolev =
      bpred_dir_create(BPredAgree, l1size, l2size, shift_width, xor);
    break;
  ...
  case BPredPerceptron:
    pred->dirpred.bimod =
      bpred_dir_create(BPredPerceptron, l1size, l2size, shift_width, 0);
    break;

case BPredTAGE:
  /* create T[0], simple 2-bit predictor */
    pred->dirpred.bimod =
	    bpred_dir_create(BPred2bit, bimod_size, 0, 0, 0);

  /* a bit messy, point the twolevel struct of dirpred at tage
  so we refrence tage using twolev*/
    pred->dirpred.twolev = 
      bpred_dir_create(BPredTAGE, l1size, 0, 0, xor);

    /* point tage base at the bimod predictor 
    base now points at T[0] */
    pred->dirpred.twolev->config.tage.base = pred->dirpred.bimod;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredAgree:
    {
      if (!l1size || (l1size & (l1size-1)) != 0)
        fatal("agree bias table size, `%d', must be non-zero and a power of two",
              l1size);
      pred_dir->config.agree.l1size = l1size;

      if (!l2size || (l2size & (l2size-1)) != 0)
        fatal("agree PHT size, `%d', must be non-zero and a power of two",
              l2size);
      pred_dir->config.agree.l2size = l2size;

      if (!shift_width || shift_width > 30)
        fatal("agree shift width, `%d', must be non-zero and positive",
              shift_width);
      pred_dir->config.agree.shift_width = shift_width;

      pred_dir->config.agree.xor = xor;

      /* allocate history shift registers */
      pred_dir->config.agree.shiftregs = calloc(l1size, sizeof(int));
      if (!pred_dir->config.agree.shiftregs)
        fatal("cannot allocate agree shift registers");

      /* allocate PHT (agree/disagree saturating counters) */
      pred_dir->config.agree.l2table = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.agree.l2table)
        fatal("cannot allocate agree PHT");

      /* allocate bias table (one bit per branch PC entry) */
      pred_dir->config.agree.bias_table = calloc(1024, sizeof(unsigned char));
      if (!pred_dir->config.agree.bias_table)
        fatal("cannot allocate agree bias table");

      /* initialize PHT counters alternating weakly agree/disagree,
       * matching the 2lev flipflop convention */
      flipflop = 1;
      for (cnt = 0; cnt < l2size; cnt++)
        {
          pred_dir->config.agree.l2table[cnt] = flipflop;
          flipflop = 3 - flipflop;
        }

      /* initialize bias table to weakly not-taken (1), matching PHT
       * warm-up convention so bias is neither maximally 0 nor 3 at start */
      flipflop = 1;
      for (cnt = 0; cnt < l1size; cnt++)
        {
          pred_dir->config.agree.bias_table[cnt] = flipflop;
          flipflop = 3 - flipflop;
        }
      break;
    }
  ...
  case BPredPerceptron:
    {
      int i;
      pred_dir->config.perceptron.table_size    = l1size;
      pred_dir->config.perceptron.history_length = l2size;
      pred_dir->config.perceptron.threshold     = (int)(1.93 * l2size + 14);
      pred_dir->config.perceptron.last_output   = 0;

      pred_dir->config.perceptron.weights = calloc(l1size, sizeof(int *));
      if (!pred_dir->config.perceptron.weights)
        fatal("cannot allocate perceptron weight table");
      for (i = 0; i < l1size; i++) {
        pred_dir->config.perceptron.weights[i] = calloc(l2size + 1, sizeof(int));
        if (!pred_dir->config.perceptron.weights[i])
          fatal("cannot allocate perceptron weights");
      }

      pred_dir->config.perceptron.history = calloc(l2size, sizeof(int));
      if (!pred_dir->config.perceptron.history)
        fatal("cannot allocate perceptron history");
      for (i = 0; i < l2size; i++)
        pred_dir->config.perceptron.history[i] = -1;

      break;
    }
    case BPredTAGE:
          {
        int lens[4] = {5, 10, 20, 40}; /* L[i] for T[1]-4 */
        /* allocate mem for each table */
        for (int i=0; i < 4; i++) {
          pred_dir->config.tage.hist_lengths[i] = lens[i];
          pred_dir->config.tage.T[i] = 
            calloc(l1size, sizeof(struct tage_entry_t));
          if (!pred_dir->config.tage.T[i])
            fatal("cannot alloc TAGE table T%d", i+1);
      }
        }
        pred_dir->config.tage.size = l1size; /* init rest */
        pred_dir->config.tage.ghist = 0;
        pred_dir->config.tage.clk = 0;
      break;
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredAgree:
    fprintf(stream,
      "pred_dir: %s: agree: %d l1-sz, %d bits/ent, %s xor, %d l2-sz\n",
      name,
      pred_dir->config.agree.l1size,
      pred_dir->config.agree.shift_width,
      pred_dir->config.agree.xor ? "" : "no",
      pred_dir->config.agree.l2size);
    break;
...
bpred_dir_lookup(struct bpred_dir_t *pred_dir,	/* branch dir predictor inst */
    ...
    struct bpred_update_t *dir_update_ptr)  	 /* needed here to update TAGE state */
{
...
case BPredAgree:
  {
    ...
    /* index into bias table by PC */
    l1index = (baddr >> MD_BR_SHIFT)
              & (pred_dir->config.agree.l1size - 1);

    /* get bias bit for this branch */
    /* (caller will store pointer into dir_update_ptr->pbias) */

    /* index PHT using history XOR'd with PC if xor flag set */
    l2index = pred_dir->config.agree.shiftregs[l1index];
    if (pred_dir->config.agree.xor)
      {
        l2index = (((l2index ^ (baddr >> MD_BR_SHIFT))
                    & ((1 << pred_dir->config.agree.shift_width) - 1))
                   | ((baddr >> MD_BR_SHIFT)
                      << pred_dir->config.agree.shift_width));
      }
    else
    ...
    l2index = l2index & (pred_dir->config.agree.l2size - 1);

    p = &pred_dir->config.agree.l2table[l2index];
    break;
  }
    ...
    case BPredPerceptron:
      {
        int i, y;
        int idx  = (baddr >> MD_BR_SHIFT)
                   & (pred_dir->config.perceptron.table_size - 1);
        int *w   = pred_dir->config.perceptron.weights[idx];
        int *hist = pred_dir->config.perceptron.history;
        int h    = pred_dir->config.perceptron.history_length;

        y = w[0];
        for (i = 0; i < h; i++)
          y += w[i+1] * hist[i];

        pred_dir->config.perceptron.last_output = y;
        p = (unsigned char *)&pred_dir->config.perceptron.last_output;
        break;
      }
        case BPredTAGE:
      {
        int i;
        int provider = -1, altprov = -1; // init to -1: choose base (bimod)

        struct bpred_dir_t *base = pred_dir->config.tage.base; // base ptr

        uint64_t ghist = pred_dir->config.tage.ghist; // load global hist

        /* compute the index and tag bits, update in bpred_update struct */
        unsigned int idx[4];
        unsigned char tag[4];
        for (i = 0; i < 4; i++) {
          int L = pred_dir->config.tage.hist_lengths[i];
          // create history mask
          uint64_t hmask = (1ULL << L) -1;
          u_int64_t h = ghist & hmask; // mask ghist
          // fold
          uint64_t folded = h;
          folded ^= h >> 10;
          folded ^= h >> 20;
          ...
          idx[i] = ((baddr >> MD_BR_SHIFT) // ignore baddr LSBs (alignment)
                    ^ folded
                    ^ (i * 432)) // random salt so each i is diff
                  & (pred_dir->config.tage.size - 1); // cuts to 10 bits

          tag[i] = ((baddr >> MD_BR_SHIFT)
                    ^ (folded >> 1) // shift by to help make the tag fucntion diff from the idx one
                    ^ (i * 131)) // differernt salt that the idx function
                  & 0xFF; // cuts to 8 bits
          
          // save in update struct
          dir_update_ptr->tage.idx[i] = idx[i];
          dir_update_ptr->tage.tag[i] = tag[i];
        }

        /* check each table to find provider and alt, start at T[4]*/
        for (i = 3; i >= 0; i--) {
        /* index the tage entry (pred, tag, u, valid)*/
          struct tage_entry_t *tage_entry = 
            &pred_dir->config.tage.T[i][idx[i]];
          
          /* if the entry is valid, and the tag == the tag we just computed */
          if (tage_entry->valid && tage_entry->tag == tag[i]) {
              if (provider == -1)
                  provider = i; // update the highest order provider 
              else if (altprov == -1) { // choose highest order altprov
                  altprov = i; // if already provider, update the altprov
                  break;
              }
          }
        }

        /* find the base prediction T[0] (always hits) */
        char *base_p = bpred_dir_lookup(base, baddr, NULL);
        char base_pred = (*base_p >= 2); // 2 bit predictor, if >=2, then pred T

        char pred, altpred;

        // if no provider (T[1]-4 not hit), then use base
        if (provider == -1) {
          pred = base_pred;
          altpred = base_pred;
        }
        else { // must be provider
        // index the prov table entry using its idx
          struct tage_entry_t *prov_entry =
            &pred_dir->config.tage.T[provider][idx[provider]];

          // sign of cntr provides pred
          char provider_pred = (prov_entry->cntr >= 0);
          // store the providers pred
          dir_update_ptr->tage.provider_pred = provider_pred;

          // do the same for the altprov
          if (altprov == -1)
              altpred = base_pred;
          else {
              struct tage_entry_t *alt_entry =
                  &pred_dir->config.tage.T[altprov][idx[altprov]];
              altpred = (alt_entry->cntr >= 0);
          }

          /* select based on useful */
          if (prov_entry->u == 0)
              pred = altpred;
          else
              pred = provider_pred;
        ...
        /* save results into update struct for usage during tage update */
        dir_update_ptr->tage.provider = provider;
        dir_update_ptr->tage.altprov  = altprov;
        dir_update_ptr->tage.pred     = pred;
        dir_update_ptr->tage.altpred  = altpred;

        /* hacky, pdir1 in bpred_lookup is expecting a 4 bit counter?
        so map the 1 bit pred to 3 or 0. it works.. */
        pred_dir->config.tage.ret_ctr = pred ? 3 : 0;
        p = (unsigned char *)&pred_dir->config.tage.ret_ctr;
      break;
  }
    default:
      panic("bogus branch direction predictor class");
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  bimod = bpred_dir_lookup (pred->dirpred.bimod, baddr,NULL);
	  twolev = bpred_dir_lookup (pred->dirpred.twolev, baddr,NULL);
	  meta = bpred_dir_lookup (pred->dirpred.meta, baddr,NULL);
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  dir_update_ptr->pdir1 =
	    bpred_dir_lookup (pred->dirpred.twolev, baddr,NULL);
	}
      break;
  case BPredAgree:                        
    if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
      {
        int l1index = (baddr >> MD_BR_SHIFT)
                      & (pred->dirpred.twolev->config.agree.l1size - 1);
        dir_update_ptr->pbias =
          (char *)&pred->dirpred.twolev->config.agree.bias_table[1024];
        dir_update_ptr->pdir1 =
          bpred_dir_lookup(pred->dirpred.twolev, baddr,NULL);
      }
  break;
  ...
  {
    dir_update_ptr->pdir1 =
      bpred_dir_lookup (pred->dirpred.bimod, baddr,NULL);
  }
    break;
    case BPredTAGE:
	    if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
      {
        // call the direction predictor on the tage instance pointed by twolev
        char *prediction = 
            bpred_dir_lookup(pred->dirpred.twolev, baddr, dir_update_ptr);

        dir_update_ptr->pdir1 = prediction; // for BTB?
        dir_update_ptr->dir.twolev = (*(unsigned char *)prediction >= 2); //update predicted dir
      }
      break;
    case BPred2bit:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
 ...
 /* if this is a jump, ignore predicted direction; we know it's taken. */
  ...
  /* for Agree predictor, interpret PHT counter relative to bias bit:
     counter >= 2 means "agree with bias", < 2 means "disagree with bias" */
  int pred_taken;
  if (pred->class == BPredAgree && dir_update_ptr->pbias)
    {
int bias_val = *(unsigned char *)dir_update_ptr->pbias;
int pht_val  = *(unsigned char *)dir_update_ptr->pdir1;
int bias_pred = (bias_val >= 2) ? 1 : 0;   // <-- extract the 1-bit prediction
pred_taken    = (pht_val >= 2) ? bias_pred : !bias_pred;
    }
  else
    {
      pred_taken = (*(dir_update_ptr->pdir1) >= 2);
    }

  if (pbtb == NULL)
    {
      /* BTB miss -- just return a predicted direction */
      return (pred_taken ? /* taken */ 1 : /* not taken */ 0);
    }
  else
    {
      /* BTB hit, so return target if it's a predicted-taken branch */
      return (pred_taken ? /* taken */ pbtb->target : /* not taken */ 0);
    }
}
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (pred->class == BPredPerceptron)
    {
      struct bpred_dir_t *p = pred->dirpred.bimod;
      int h      = p->config.perceptron.history_length;
      int thresh = p->config.perceptron.threshold;
      int idx    = (baddr >> MD_BR_SHIFT)
                   & (p->config.perceptron.table_size - 1);
      int *w    = p->config.perceptron.weights[idx];
      int *hist = p->config.perceptron.history;
      int y     = p->config.perceptron.last_output;
      int t     = taken ? 1 : -1;
      int i;

      if ((!taken && y >= 0) || (taken && y < 0) || (abs(y) <= thresh))
        {
          w[0] += t;
          if (w[0] >  127) w[0] =  127;
          if (w[0] < -127) w[0] = -127;
...
return;
  ...
  {
    struct bpred_dir_t *p = pred->dirpred.twolev;
    int l1index, shift_reg;
    int bias_pred, bias_val;

if (dir_update_ptr->pbias)  
    {
        /* 1. Determine what bias counter currently predicts */
        bias_val  = *(unsigned char *)dir_update_ptr->pbias;
        bias_pred = (bias_val >= 2) ? 1 : 0;

        /* update bias counter */
        if (taken)
          { if (bias_val < 3) ++*(unsigned char *)dir_update_ptr->pbias; }
        else
          { if (bias_val > 0) --*(unsigned char *)dir_update_ptr->pbias; }

        /* 3. Update PHT counter */
        ...
        /* 4. Update L1 history shift register */
        l1index = (baddr >> MD_BR_SHIFT)
                  & (p->config.agree.l1size - 1);
        {
            int agree_bit = (!!taken == bias_pred) ? 1 : 0;
            shift_reg = (p->config.agree.shiftregs[l1index] << 1) | agree_bit;
        }
        p->config.agree.shiftregs[l1index] =
            shift_reg & ((1 << p->config.agree.shift_width) - 1);

    }
    /* fall through to BTB update below */
  }
  ...
  {
    // point at T[0]-4
    struct bpred_dir_t *tage = pred->dirpred.twolev;
    struct bpred_dir_t *bimod = pred->dirpred.bimod;

    // populate all update structure fields
    int provider = dir_update_ptr->tage.provider;
    char pred = dir_update_ptr->tage.pred;
    char altpred = dir_update_ptr->tage.altpred;
    char provider_pred = dir_update_ptr->tage.provider_pred;

    int i;
    unsigned int idx[4];
    unsigned char tag[4];

    /* repopulate idx and tag */
    for (i = 0; i < 4; i++) {
      idx[i] = dir_update_ptr->tage.idx[i];
      tag[i] = dir_update_ptr->tage.tag[i];
    }

    /* first, the base predictor is always updated */
    {
      unsigned char *cntr = (unsigned char *)bpred_dir_lookup(bimod, baddr, NULL);
    ...
    /* next, update the provider, if it exists */
    if (provider != -1) {
      struct tage_entry_t *prov_entry =
        &tage->config.tage.T[provider][idx[provider]];

      /* 3 bit signed cntr [-4, +3] */
      if (taken) {
        if (prov_entry->cntr < 3) prov_entry->cntr++;
      } else {
        if (prov_entry->cntr > -4) prov_entry->cntr--;
      }

      /* useful bit update, 2 bit unsigned */
      if (provider_pred != altpred) {
        if (provider_pred == taken) {
          if (prov_entry->u < 3) prov_entry->u++;
        } else {
          if (prov_entry->u > 0) prov_entry->u--;
        }
      }
    }

    /* allocate new table entry on a mispredict */
    /* the allocation goes to a higher order table,
    if it has a open location */
    if (pred != taken) {
      for (i = provider + 1; i < 4; i++) {
        // get an entry
        struct tage_entry_t *tage_entry =
            &tage->config.tage.T[i][idx[i]];

        // if the entry is not useful or invalid, allocate here
        if (tage_entry->u == 0 || tage_entry->valid == 0) {
          tage_entry->tag = tag[i];
          tage_entry->cntr = taken ? 0 : -1;
          tage_entry->u   = 0;
          tage_entry->valid = 1;
          break;
        }
  ...
  /* update the ghist */
    ...
    tage->config.tage.ghist =
      ((tage->config.tage.ghist << 1) | (taken ? 1 : 0)) // shift in outcome
      & ((1ULL << 40) - 1); // 40 bit mask

    
    /* update clk, and periodically perform graceful reset of u */
    if (++tage->config.tage.clk % 256000 == 0) {
      int t, j;
      for (t = 0; t < 4; t++) {
        for (j = 0; j < tage->config.tage.size; j++) {
          tage->config.tage.T[t][j].u >>= 1;
        }
      }
    }
  }
  ...
  /* update state (but not for jumps) -- agree already updated PHT above */
  if (dir_update_ptr->pdir1 && pred->class != BPredAgree)
    {
      if (taken)
	{
