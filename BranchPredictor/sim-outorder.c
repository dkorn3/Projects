...
static int agree_config[4] = { 1, 1024, 8, 1 };  /* l1size, l2size, hist_size, xor */
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-bpred",
    "branch predictor type {nottaken|taken|perfect|bimod|2lev|comb|agree|percept|tage}",
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  {
    if (agree_nelt != 4)
      fatal("bad agree predictor config (<l1size> <l2size> <hist_size> <xor>)");
    if (btb_nelt != 2)
      fatal("bad btb config (<num_sets> <associativity>)");

    pred = bpred_create(BPredAgree,
                        /* bimod table size */0,
                        /* l1size (bias table) */agree_config[0],
                        /* l2size (PHT)       */agree_config[1],
                        /* meta table size */0,
                        /* history reg size */agree_config[2],
                        /* history xor addr */agree_config[3],
                        /* btb sets */btb_config[0],
                        /* btb assoc */btb_config[1],
                        /* ret-addr stack size */ras_size
                      );
...
else if (!mystricmp(pred_type, "percept")) 
  {
    pred = bpred_create(BPredPerceptron,
                        /* bimod table size */0,
                        /* 2lev l1 size (table size) */256,
                        /* 2lev l2 size (history length) */24,
                        /* meta table size */0,
                        /* history reg size */24,
                        /* history xor address */0,
                        /* btb sets */btb_config[0],
    ...
    else if (!mystricmp(pred_type, "tage")) 
  {
	pred = bpred_create(BPredTAGE,
			1024, /* bimod size for T0 */
			1024, /* 1024 entries per T1-T4 */
			0,    /* l2 size not needed */
			0,    /* meta size not needed */
			40,   /* history size */
			0,    /* xor */
